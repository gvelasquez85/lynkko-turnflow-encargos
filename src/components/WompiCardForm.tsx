...
export function WompiCardForm({ amountCents, currency = 'COP', onSuccess, onCancel, newEst, newAdv }: Props) {
    ...
    try {
      ...
      const tokenRes = await fetch(`${WOMPI_BASE}/tokens/cards`, {
        ...
        body: JSON.stringify({
          ...
          cvc, exp_month: expMonth, exp_year: expYear,
          card_holder: cardHolder.trim(),
        }),
      })
      const tokenJson = await tokenRes.json()
      if (!tokenRes.ok || !tokenJson.data?.id) throw new Error(wompiExtractError(tokenJson, 'Datos de tarjeta inválidos'))
      const cardToken: string = tokenJson.data.id
      const setupRes = await fetch('/api/billing/setup-card', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cardToken, acceptanceToken: acceptance.acceptanceToken,
          personalDataToken: acceptance.personalDataToken, currency,
          ...(newEst ? { newEst } : {}), ...(newAdv ? { newAdv } : {}),
        }),
      })
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Button type="submit" disabled={loading || !acceptance || !acceptTerms} className="w-full">
        {loading
      ...
      {onCancel && (
        <button type="button" onClick={onCancel} className="text-sm text-gray-400 hover:text-gray-600 text-center w-full py-1">
          Cancelar
        </button>
      )}
      <p className="text-[10px] text-gray-400 text-center flex items-center justify-center gap-1">
        <Lock size={10} /> Pago seguro procesado por{' '}
        <a href="https://wompi.co" target="_blank" rel="noopener noreferrer" className="font-semibold text-gray-500 hover:text-gray-700">Wompi</a>
        {' '}· Visa · Mastercard
      </p>
    </form>
  )
