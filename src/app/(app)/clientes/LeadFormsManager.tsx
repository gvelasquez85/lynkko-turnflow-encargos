...
export default function LeadFormsManager({ brandId }: { brandId: string }) {
  ...
  const loadForms = useCallback(async () => {
    const res = await fetch('/api/admin/clientes/lead-forms')
    if (res.ok) {
  ...
  async function createForm() {
    ...
    const res = await fetch('/api/admin/clientes/lead-forms', {
      ...
      body: JSON.stringify({ name: newName.trim(), fields: newFields }),
    })
    if (res.ok) {
  ...
  async function toggleActive(form: LeadForm) {
    const res = await fetch(`/api/admin/clientes/lead-forms/${form.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteForm(id: string) {
    ...
    const res = await fetch(`/api/admin/clientes/lead-forms/${id}`, { method: 'DELETE' })
      ...
      {showCreate && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 mb-6">
          ...
          <div className="flex gap-2 mb-5">
            <input type="text" value={customFieldLabel} onChange={e => setCustomFieldLabel(e.target.value)}
            ...
            <button onClick={addCustomField} disabled={!customFieldLabel.trim()}
          ...
          <div className="flex items-center gap-2">
            <button onClick={createForm} disabled={creating || !newName.trim()}
              className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors">
              {creating ? 'Creando...' : 'Crear formulario'}
            </button>
            <button onClick={() => { setShowCreate(false); setNewName(''); setNewFields(DEFAULT_FIELDS) }}
          ...
          {forms.map(form => (
            <div key={form.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {form.fields.length} campos · Creado {new Date(form.createdAt).toLocaleDateString('es-CO')}
                  </p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={() => copyToClipboard(getFormUrl(form.id), `link-${form.id}`)}
                ...
                <button onClick={() => copyToClipboard(getEmbedCode(form.id), `embed-${form.id}`)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                  {copied === `embed-${form.id}` ? <Check size={13} className="text-green-500" /> : <Code size={13} />}
                  {copied === `embed-${form.id}` ? 'Copiado' : 'Copiar embed'}
                </button>
                <a href={getFormUrl(form.id)} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
