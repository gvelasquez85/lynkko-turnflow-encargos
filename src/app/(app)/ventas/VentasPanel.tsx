...
function getFlow(sale: Sale) {
  ...
  if (!sale.sourceQuoteId) return MANUAL_FLOW
  return QUOTE_FLOW
}
...
export default function VentasPanel({
  ...
  pendingSales: initialPending,
  establishments,
  waTemplates = [],
  brandName = 'Tu negocio',
  hasFacturacion = false,
  hasContabilidad = false,
}: Props) {
  ...
  async function sendConfirmationEmail(sale: Sale) {
    ...
    setEmailResult(null)
    try {
      const res = await fetch(`/api/ventas/${sale.id}/send-confirmation`, {
        method: 'POST',
  ...
  const totalRevenue = completedSales.reduce((s, x) => s + Number(x.total ?? 0), 0)
  ...
  const allTodaySales = useMemo(() =>
    recentSales.filter(s => s.status !== 'cancelled' && new Date(s.createdAt).toDateString() === todayStr),
    [recentSales, todayStr])
  // Revenue = completed/invoiced only
  const todaySales = completedSales.filter(s => new Date(s.createdAt).toDateString() === todayStr)
  ...
  const pendingRevenue = pendingSalesKPI.reduce((s, x) => s + Number(x.total ?? 0), 0)
  ...
  function openSaleWa(sale: Sale, category: 'sale_receipt' | 'sale_pending_payment') {
    ...
    if (!phone) return
    const fecha = new Date(sale.createdAt).toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })
  ...
  async function saveNotes() {
    if (!openSale) return
    setSaving(true)
    await fetch(`/api/ventas/${openSale.id}/notes`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function changeStatus(newStatus: string) {
    ...
    try {
      await fetch(`/api/ventas/${openSale.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: newStatus, notify: notifyClient }),
      })
    ...
    <div className="flex flex-col lg:flex-row gap-5">
      ...
      <div className={`flex-1 min-w-0 ${openSaleId ? 'hidden lg:block' : ''}`}>
              ...
              {pendingSales.map(sale => (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">{fmtDate(sale.createdAt)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { href: '/ventas/inventario', icon: Package, label: 'Inventario', desc: 'Gestiona productos y stock', color: 'bg-amber-50 dark:bg-amber-900/30 border-amber-200 dark:border-amber-700 hover:border-amber-300 dark:hover:border-amber-600', iconColor: 'bg-amber-100 text-amber-700' },
            { href: '/ventas/cotizaciones', icon: FileCheck, label: 'Cotizaciones', desc: 'Crea y da seguimiento', color: 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-700 hover:border-blue-300 dark:hover:border-blue-600', iconColor: 'bg-blue-100 text-blue-700' },
            { href: '/ventas/nueva-venta', icon: ShoppingCart, label: 'Nueva venta', desc: 'Registrar venta directa', color: 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-700 hover:border-emerald-300 dark:hover:border-emerald-600', iconColor: 'bg-emerald-100 text-emerald-700' },
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            <Link href="/reportes/ventas" className="text-xs text-indigo-600 hover:underline flex items-center gap-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {sale.establishmentId && (
                          <span className="text-[10px] text-gray-400 dark:text-gray-500">{estMap[sale.establishmentId]}</span>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden sticky top-6">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800">
              <div>
                ...
                <p className="font-bold text-gray-900 dark:text-gray-100">{openSale.customerName ?? 'Sin cliente'}</p>
              </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <span className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500">
                      <Calendar size={11} /> {fmtDateFull(openSale.createdAt)}
                    </span>
                  </div>

                  {/* Customer info */}
                  {openSale.customerName && (
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3 text-sm space-y-1">
                      <div className="flex items-center gap-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        <User size={13} className="text-gray-400 dark:text-gray-500" /> {openSale.customerName}
                      </div>
                      {openSale.customerEmail && (
                        <p className="text-gray-500 dark:text-gray-400 text-xs">{openSale.customerEmail}</p>
                      )}
                    </div>
                  )}

                  {/* Establishment */}
                  {openSale.establishmentId && estMap[openSale.establishmentId] && (
                    <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                      <Building2 size={12} className="text-gray-400 dark:text-gray-500" /> {estMap[openSale.establishmentId]}
                    </div>
                  )}
                            ...
                            {panelItems.map((it, i) => (
                              <tr key={i}>
                                <td className="py-2 font-medium text-gray-900 dark:text-gray-100 text-xs">
                                  {it.productName}
                                  {it.productSku && <span className="text-gray-400 dark:text-gray-500 ml-1">({it.productSku})</span>}
                                </td>
                                <td className="py-2 text-center text-gray-500 dark:text-gray-400 text-xs">{it.qty}</td>
                                <td className="py-2 text-right text-gray-500 dark:text-gray-400 text-xs">{fmt(it.unitPrice)}</td>
                                <td className="py-2 text-right font-semibold text-xs text-gray-900 dark:text-gray-100">{fmt(it.lineTotal)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    return (
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">
                          {openSale.sourceQuoteId ? 'Desde cotización' : 'Venta directa'}
                        </p>
