...
import { Plus, Store, Edit2, ToggleLeft, ToggleRight, QrCode, Building2, Clock, CheckCircle, Settings2, Trash2, AlertTriangle } from 'lucide-react'
...
export function EstablishmentsManager({ establishments: initial, brands, defaultBrandId, ticketStats = {}, hasQueue = false, isSuperAdmin, maxEstablishments }: Props) {
  ...
  function getFeatures(est: Establishment): Record<string, boolean> {
    return est.features ?? { queue: true, appointments: false, surveys: false, menu: false }
  }

  async function saveFeatures(est: Establishment, features: Record<string, boolean>) {
    setFeaturesLoading(true)
    const res = await fetch(`/api/admin/brand/establishments/${est.id}`, {
      method: 'PATCH',
    ...
    if (res.ok) {
      ...
      setEstablishments(es => es.map(e => e.id === est.id ? data : e))
    }
  ...
  const filteredEstablishments = selectedBrandId
    ? establishments.filter(e => e.brandId === selectedBrandId)
    : establishments
  ...
  function openNew() {
    if (!isSuperAdmin && maxEstablishments !== undefined) {
      const brandEsts = establishments.filter(e => e.brandId === selectedBrandId)
  ...
  function openEdit(e: Establishment) {
    setEditing(e)
    setFormError('')
    setForm({ name: e.name, slug: e.slug, address: e.address || '', brand_id: e.brandId })
    setShowForm(true)
  }
  ...
  async function handleSave() {
    ...
    setLoading(true)
    if (editing) {
      const res = await fetch(`/api/admin/brand/establishments/${editing.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: form.name, slug: form.slug, address: form.address }),
      })
      if (!res.ok) {
      ...
      const data = await res.json()
      setEstablishments(es => es.map(e => e.id === editing.id ? data : e))
    } else {
      const res = await fetch('/api/admin/brand/establishments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: form.name, slug: form.slug, address: form.address }),
      })
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/admin/brand/establishments/${deleteConfirm.id}`, { method: 'DELETE' })
        ...
        <Button
          ...
          disabled={
            ...
            (!isSuperAdmin && maxEstablishments !== undefined &&
              establishments.filter(e => e.brandId === selectedBrandId).length >= maxEstablishments)
          }
          title={
            !isSuperAdmin && maxEstablishments !== undefined &&
            establishments.filter(e => e.brandId === selectedBrandId).length >= maxEstablishments
      ...
      {!isSuperAdmin && maxEstablishments !== undefined &&
        ...
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5">
          ...
          <p className="text-sm text-amber-800 flex-1">
            ...
            <a href="/configuracion/brand?tab=membership" className="underline font-semibold hover:text-amber-900">
