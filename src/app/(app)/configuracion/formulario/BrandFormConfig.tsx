...
export function BrandFormConfig({
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const res = await fetch('/api/admin/marca/form-config', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ formFields: fields, dataPolicyText: policy || null }),
    })
    setSaving(false)
    if (!res.ok) {
      const data = await res.json().catch(() => ({}))
      setError(data.error || 'No se pudo guardar. Verifica que tienes permisos.')
      return
    }
    setSaved(true)
