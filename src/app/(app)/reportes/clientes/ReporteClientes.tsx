...
export function ReporteClientes({ customers, establishments }: {
  ...
  const newInPeriod = useMemo(() =>
    base.filter(c => new Date(c.firstVisitAt) >= range.from && new Date(c.firstVisitAt) <= range.to),
    [base, range])

  // Returning in period (last visit in range, but first visit before range)
  const returningInPeriod = useMemo(() =>
    base.filter(c =>
      new Date(c.lastVisitAt) >= range.from &&
      new Date(c.lastVisitAt) <= range.to &&
      new Date(c.firstVisitAt) < range.from
    ), [base, range])

  const frequent = useMemo(() => base.filter(c => c.totalVisits >= 4), [base])
  const loyal    = useMemo(() => base.filter(c => c.totalVisits >= 10), [base])

  const atRisk = useMemo(() =>
    base.filter(c => {
      const days = Math.floor((Date.now() - new Date(c.lastVisitAt).getTime()) / 86400000)
      return days > 60 && c.totalVisits >= 2
    }), [base])
  ...
  const byEst = useMemo(() => {
    ...
    base.forEach(c => c.establishmentIds.forEach(eid => { map[eid] = (map[eid] ?? 0) + 1 }))
  ...
  const visitDist = useMemo(() => [
    { label: 'Nuevos (1 visita)',  count: base.filter(c => c.totalVisits === 1).length, color: 'bg-blue-500' },
    { label: 'Ocasionales (2–3)', count: base.filter(c => c.totalVisits >= 2 && c.totalVisits <= 3).length, color: 'bg-amber-500' },
    { label: 'Frecuentes (4–9)',  count: base.filter(c => c.totalVisits >= 4 && c.totalVisits <= 9).length, color: 'bg-green-500' },
    { label: 'Fieles (10+)',      count: base.filter(c => c.totalVisits >= 10).length, color: 'bg-purple-500' },
  ], [base])
          ...
          {atRisk.slice(0, 5).map(c => (
            <div key={c.id} className="flex items-center justify-between py-1.5 border-t border-gray-50 first:border-0">
              ...
              <p className="text-xs text-gray-400 shrink-0 ml-2">
                {Math.floor((Date.now() - new Date(c.lastVisitAt).getTime()) / 86400000)}d sin visitar
              </p>
            </div>
          ))}
        ...
        <div className="bg-white rounded-xl border border-red-200 p-5">
          ...
          {lost.slice(0, 5).map(c => (
            <div key={c.id} className="flex items-center justify-between py-1.5 border-t border-gray-50 first:border-0">
              <p className="text-xs font-medium text-gray-700 truncate">{c.name}</p>
              <p className="text-xs text-gray-400 shrink-0 ml-2">
                {Math.floor((Date.now() - new Date(c.lastVisitAt).getTime()) / 86400000)}d
              </p>
            </div>
          ))}
