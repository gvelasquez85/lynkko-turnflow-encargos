...
export async function POST(req: NextRequest) {
  ...
  if (!encargo) {
    return NextResponse.json({ error: 'No se pudo crear el encargo' }, { status: 500 })
  }

  if (items.length > 0) {
    await db.insert(encargoItems).values(
      items.map((it, idx) => ({
    ...
    ).catch(err => console.warn('[encargos/create] encargoItems error (non-fatal):', err.message))
  }
  ...
  if (!sale) {
    return NextResponse.json({ ok: true, encargoId: encargo.id, saleCreated: false })
  }
  ...
  await db.update(encargos).set({ saleId: sale.id }).where(eq(encargos.id, encargo.id))

  return NextResponse.json({ ok: true, encargoId: encargo.id, saleId: sale.id, saleCreated: true })
}
